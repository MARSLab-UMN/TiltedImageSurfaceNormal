...
class Warping2DOFAlignment:
    ...
    def _build_homography_quaternion(self, q):
        Cg_R_C = torch.zeros((q.shape[0], 3, 3), device=q.device, dtype=torch.float)
        q = q.view(q.shape[0], 4, 1)
        for i in range(Cg_R_C.shape[0]):
            skewsymm_Cg_q_C = self._skewsymm(q[i][0:3].view(-1))
            Cg_R_C[i] = self.I3 + 2. * q[i][3] * skewsymm_Cg_q_C + \
                                      2. * skewsymm_Cg_q_C @ skewsymm_Cg_q_C
        Cg_H_C = self.K @ Cg_R_C @ self.K_inv
        C_R_Cg = Cg_R_C.permute(0, 2, 1)
        Cg_H_C_inv = self.K @ C_R_Cg @ self.K_inv
    ...
    def image_sampler_forward_inverse_quaternion(self, q):
        Cg_H_C, Cg_R_C, Cg_H_C_inv = self._build_homography_quaternion(q)
        Cg_H_C = Cg_H_C.type(torch.float)
        Cg_H_C_inv = Cg_H_C_inv.type(torch.float)
        C_R_Cg = Cg_R_C.permute(0, 2, 1)

        grid_sampler = torch.cuda.FloatTensor(q.shape[0], self.H, self.W, 2).fill_(0)
        inv_grid_sampler = torch.cuda.FloatTensor(q.shape[0], self.H, self.W, 2).fill_(0)
        C_R_Cg_ret = torch.zeros_like(C_R_Cg)
        for i in range(q.shape[0]):
            ...
            if scale_sigma < 0.8 or scale_sigma > 2.2:
                C_R_Cg_ret[i] = self.I3
                grid_sampler[i, :, :, 0] = torch.reshape(1. / (self.W / 2) * (self.XX - self.cx),
                                                         (self.W, self.H)).t()
                grid_sampler[i, :, :, 1] = torch.reshape(1. / (self.H / 2) * (self.YY - self.cy),
                                                         (self.W, self.H)).t()
                inv_grid_sampler[i, :, :, 0] = torch.reshape(1. / (self.W / 2) * (self.XX - self.cx),
                                                             (self.W, self.H)).t()
                inv_grid_sampler[i, :, :, 1] = torch.reshape(1. / (self.H / 2) * (self.YY - self.cy),
            ...
            inv_C1p_x = kw * (inv_C1p_projection[0, :] - px_min)
            inv_C1p_y = kh * (inv_C1p_projection[1, :] - py_min)
            inv_grid_sampler[i, :, :, 0] = torch.reshape(1. / (self.W / 2) * (inv_C1p_x - self.cx),
                                                         (self.W, self.H)).t()
            inv_grid_sampler[i, :, :, 1] = torch.reshape(1. / (self.H / 2) * (inv_C1p_y - self.cy),
                                                         (self.W, self.H)).t()
            C_R_Cg_ret[i] = C_R_Cg[i]
        # y = torch.nn.functional.grid_sample(x, grid_sampler, padding_mode='zeros', mode=interp_mode)
        return C_R_Cg_ret, grid_sampler, inv_grid_sampler
